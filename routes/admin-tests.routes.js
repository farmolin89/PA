// ===================================================================
// Файл: routes/admin-tests.routes.js (ПОЛНАЯ ИТОГОВАЯ ВЕРСИЯ ПОСЛЕ РЕФАКТОРИНГА)
// Описание: Маршруты для администрирования тестов. Выполняет роль
// контроллера, вызывая бизнес-логику из сервисного слоя.
// ===================================================================
...
module.exports = (knex) => {
    ...
    router.get('/results/:resultId/protocol', async(req, res, next) => {
        try {
            const protocolData = await protocolService.getProtocol(req.params.resultId);
    ...
    router.get('/results/:resultId/review', async(req, res, next) => {
        try {
            const questions = await reviewService.getPending(req.params.resultId);
            res.json(questions);
        } catch(error) { next(error); }
    });
    ...
    // ===================================================================
    // +++ НОВЫЙ МАРШРУТ ДЛЯ ДАННЫХ ГРАФИКА +++
    // ===================================================================
